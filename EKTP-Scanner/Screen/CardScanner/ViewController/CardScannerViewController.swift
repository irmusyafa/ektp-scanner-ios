...
class CardScannerViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let destination = segue.destination as? CardScannerErrorViewController {
            destination.capturedImage = self.capturedImage
        }
        if let destination = segue.destination as? CardScannerResultViewController {
...
extension CardScannerViewController: FaceDetectionUtilityDelegate {
